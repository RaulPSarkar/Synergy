...
import drug_pairs_build_functions
...
def main(settings):
    ...
    if settings['use_data_generators']:
        ...
        train_dataset = DataGenerator(y_filepath=settings['train_response_filepath'],
                                      output_col=['Delta Xmid', 'Delta Emax'],
    ...
    if settings['use_data_generators']:
        #weights = class_weight.compute_class_weight('balanced', np.unique(val_dataset.y), val_dataset.y)
       ...
       # weights = class_weight.compute_class_weight('balanced', np.unique(train_dataset.y), train_dataset.y)
       # weights = {i : weights[i] for i in range(2)}
        ...
        history = best_model.fit(train_dataset.X_dict, train_dataset.y, epochs=settings['epochs'],
                                 ...
                                 callbacks=[EarlyStopping(patience=15, restore_best_weights=True),
                                            CSVLogger(os.path.join(output_dir, 'training.log'))],
                                 validation_data=(
                                     val_dataset.X_dict, val_dataset.y), validation_batch_size=64)#, class_weight=weights)
